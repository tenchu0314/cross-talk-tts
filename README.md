...
3. **ADVゲーム風のダイナミックな演出**:
   - ネオンが光るハイテク討論スタジオをイメージしたダークテーマ of 背景デザイン。
   - 会話の進行と `emotion` (default/serious/angry) に連動して、キャラクター（Speaker1とSpeaker2）の表情別立ち絵がスムーズに切り替わります。
   - 発言中のキャラクターは拡大かつ明るくハイライトされ、非発言者は自動で暗くなり後退するアニメーション演出。
   - 討論中に Gemini が使用した検索キーワードを画面上部にリアルタイム表示。
4. **堅牢な非同期処理 & 中断制御**:
   - 討論を途中で「終了」した際に、実行中の非同期フェッチリクエストや Audio ロードイベントをすべて安全に破棄するセッションID管理を導入。リソースの安全な解放と不要なリクエストの抑制を行います。
...
## 🎨 フロントエンドの設計とコンポーネント構成

フロントエンドは、当初のモノリシックな構造（900行超の単一 `App.tsx`）から、**責務の分離 (Separation of Concerns)** に基づいた機能ベースのモジュール構成へと大幅にリファクタリングされています。

### 主要な構成要素

- **カスタムHook (`src/hooks/useDebatePlayer.ts`)**:
  - 音声再生のライフサイクル（再生・一時停止・スキップ）、TTS生成キュー管理（同時接続制御）、バッファリング進行状況の推定アルゴリズムをすべて集約しています。
- **画面コンポーネント (`src/components/screens/`)**:
  - 各状態に対応する画面を個別のコンポーネント（ヘルスチェック、接続エラー、議題入力、生成中、ADV討論画面）に分割し、ロジックと描画の結合を疎にしています。
- **UIパーツ (`src/components/ui/`)**:
  - セリフボックス（`DialogueBox`）、立ち絵（`CharacterSprite`）、再生コントロール（`PlaybackControls`）などの最小単位のUIパーツを分離し、可読性と再利用性を向上させています。
- **スタイル設計**:
  - グローバルなCSS変数やレイアウト、共通のアニメーションキーフレームは `App.css` に定義し、コンポーネント固有のスタイルは対応する各 `.css` ファイルへとカプセル化（コロケーション）しています。

```
src/
├── types.ts                                    # 共通型定義（討論、画面状態、バッファ）
├── constants.ts                                # 各種定数（アバター画像パス、再生パラメータ等）
├── hooks/
│   └── useDebatePlayer.ts                      # 討論プレイヤーのコアロジック Hook
├── components/
│   ├── screens/                                # 画面単位のコンポーネント
│   │   ├── HealthCheckScreen.tsx / .css
│   │   ├── ConnectionErrorScreen.tsx / .css
│   │   ├── InputScreen.tsx / .css
│   │   ├── GeneratingScreen.tsx / .css
│   │   └── DebatePlayerScreen.tsx / .css
│   └── ui/                                     # UIパーツコンポーネント
│       ├── CharacterSprite.tsx / .css
│       ├── DialogueBox.tsx / .css
│       ├── PlaybackControls.tsx / .css
│       ├── ProgressTrack.tsx / .css
│       ├── BufferingOverlay.tsx / .css
│       └── DebateEndOverlay.tsx / .css
├── App.tsx                                      # 画面ルーティングのみを担当するルート
├── App.css                                      # グローバルテーマ・共通アニメーション
```
...
- Irodori-TTS-Server の `voices/` フォルダに、以下の **2つのサンプル音声ファイル** が配置されていること（**ファイル名は完全一致が必須**）：
  - `Speaker1.wav` … Speaker1（デフォルト: クレア）の声のサンプル
  - `Speaker2.wav` … Speaker2（デフォルト: カレン）の声のサンプル
